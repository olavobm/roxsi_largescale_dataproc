...
% for processing data, then use this
% string to create subfolders where
% each segment will be stored
dir_segment = 'segment_';
...
% %                 sigL1.cor5{i3} = dataread_aux.Data.IBurst_CorBeam5(lin_proclims_beam5time_aux, lin_verticalrange);
    ...
    % %         cor5_aux = vel5_aux;
            ...
            for i3 = 1:size(sigL1.vel5, 2)
                
                %
                vel5_aux(:, i3) = interp1(sigL1.dtime5, ...
                                          sigL1.vel5(:, i3), ...
                                          sigL1.dtime);
    % %             %
    % %             amp5_aux(:, i3) = interp1(sigL1.dtime5, ...
    % %                                       sigL1.amp5(:, i3), ...
    % %                                       sigL1.dtime);
    % %             %
    % %             cor5_aux(:, i3) = interp1(sigL1.dtime5, ...
    % %                                       single(sigL1.cor5(:, i3)), ...
    % %                                       sigL1.dtime);
            end
    ...
    % %         sigL1.cor5 = cor5_aux;
        ...
        %
        for i3 = 1:length(list_all_fields)
    
            % Only interpolate fields that are NOT contained in list_time_vars
            % and are arrays with expected number of points along time
            if ~any(contains(list_time_vars, list_all_fields{i3})) && ...
               (size(sigL1.(list_all_fields{i3}), 1) == Nlengthtimeseries)
               
                %
                sigL1.(list_all_fields{i3}) = ...
                                interp1(sigL1.dtime, ...              
                                        sigL1.(list_all_fields{i3}), ...
                                        dtime_grid);
    ...
    % %             sigL1.(list_variables_aux{i3}) = sigL1.(list_variables_aux{i3})(:);
    ...
    % %     % may remove the edges.
        ...
        for i3 = 1:length(list_all_fields)
            %
            if (size(sigL1.(list_all_fields{i3}), 1)==Nptstime) && ...
               (size(sigL1.(list_all_fields{i3}), 2)==Nptsbins)
                %
                sigL1.(list_all_fields{i3}) = sigL1.(list_all_fields{i3})(:, lin_verticalrange); 
    
                % Transpose matrices so that row dimension is along bins
                sigL1.(list_all_fields{i3}) = sigL1.(list_all_fields{i3}).';
            end
        end
    ...
    % %     Nbins = length(sigL1.zhab);   % not used
        ...
        %
        for i3 = 1:length(sigL1.dtime)
            %
            ind_withinocean_aux = find((zhab_halfstep < sigL1.bottomdepthfrompres(i3)), 1, 'last');
            %
            ind_abovesurface(1:ind_withinocean_aux, i3) = NaN;
        end
        %
    ...
    % %     sigL1.Wbeam5 = NaN(size(sigL1.vel1));
        ...
        if sigL1.l5beams
            ...
            %
            for i3 = 1:size(indbreak_rot, 2)
    
                %
                disp(['--- Transformation in chunk ' num2str(i3) ' out of ' num2str(size(indbreak_rot, 2)) ' ---'])
                %
                ind_sub_aux = indbreak_rot(1, i3) : indbreak_rot(2, i3);
        ...
        %
        for i3 = 1:length(sigL1.averaged.dtime)
            %
            ind_withinocean_aux = find(((sigL1.zhab + sigL1.binsize) < sigL1.averaged.bottomdepthfrompres(i3)), 1, 'last');
            %
            ind_avg_abovesurface(1:(ind_withinocean_aux-1), i3) = NaN;    % and remove another bin because of waves
        end
        %
    ...
    % %     list_fields_aux = {'u', 'v', 'w', 'amp1', 'amp2', 'amp3', 'amp4'};
        ...
        for i3 = 1:length(list_fields_aux)
            for i4 = 1:length(sigL1.zhab)
                %
                sigL1.averaged.(list_fields_aux{i3})(i4, :) = ...
                                 time_smooth_reg(sigL1.dtime, ...
                                                 sigL1.(list_fields_aux{i3})(i4, :), ...
                                                 sigL1.averaged.dt, ...
                                                 sigL1.averaged.dtime([1, end]));
            end
    
            % Remove the above the surface
            sigL1.averaged.(list_fields_aux{i3})(ind_avg_abovesurface) = NaN;
    ...
    % %     end
        ...
        %
        for i3 = 1:length(list_vars_move)
            if isfield(sigL1, list_vars_move{i3})
                sigL1 = rmfield(sigL1, list_vars_move{i3});
            end
        end
        %
