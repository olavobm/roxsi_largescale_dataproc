function [Sd,D,Sw,Fcof,Gwt,Sxyn,Sxy,k,Hw,Hw_initial] = dat2dspec_frequency_cut(xn,pos,h,nfft,nt,method,varargin,cutoff)
...
%      Fcof = Fourier coeffiSdcients of the spreading function D(w,theta)
...
Sf = Sf.';
%kw = kw(:)';
% figure(2); clf; plot(Hw(3,:))
%-------------------------------------------------------------------------------
%Estimate frequency spectrum for the surface elevation from the bfs timeseries
%--------------------------------------------------------------------------------
% keyboard

% PC EDITS to apply a cutoff frequency
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ind = find(fi > cutoff, 1)+1;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SfBest = bfsspec(Sf(:,1:ind-1),Hw(:,1:ind-1),pos,bfs);
...
% OBM EDIT: with the above, when k subsets Gwt,
% its dimension becomes inconsistent with Sxyn
lbad = (SfBest==0);
%
Gwt(:, :, lbad) = NaN;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    DS = mlm(Sxyn(:,:,1:ind-1),Gwt(:,:,1:ind-1),thetai,fi(1:ind-1),k,opt);
   DS = mlm(Sxyn,Gwt,thetai,fi(1:ind-1),k,opt);
 case {'imlm'} % iterative maximum likelihood method
   DS = imlm(Sxyn(:,:,1:ind-1),Gwt(:,:,1:ind-1),thetai,fi(1:ind-1),k,opt);
  case 'mem' , %Maximum entropy method
    DS = mem(Sxyn(:,:,1:ind-1),Gwt(:,:,1:ind-1),thetai,fi(1:ind-1),k);
  case 'emem' , %Extended Maximum entropy method      
    DS = emem(Sxyn(:,:,1:ind-1),Gwt(:,:,1:ind-1),thetai,fi(1:ind-1),k,opt);
  case 'bdm', %   
    DS = bdm(Sxyn(:,:,1:ind-1),Gwt(:,:,1:ind-1),thetai,fi(1:ind-1),k,opt);
