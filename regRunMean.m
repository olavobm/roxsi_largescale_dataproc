function [xmean, indcenter, xstddev] = regRunMean(npts, x, slidepts, wndwtype, indlims)
% [xmean, indcenter, xstddev] = REGRUNMEAN(npts, x, slidepts, wndwtype, indlims)
%
%   inputs
%       - npts: number of points averaged together.
%       - x: a vector of data to be bin-averaged.
%       - slidepts: number of points to slide when averaging.
%       - wndwtype: type of window weighting (input for Matlab function window.m).
%       - indlims (optional): 1x2 vector with the indices of the edges
%                             of x to trim it before bin-averaging. If
%                             not given, indlims is set to [1, length(x)].
%
%   outputs
%       - xmean: bin-averaged x.
%       - indcenter: indices of the centers of the windowing (which is
%                    useful if you have a time vector outside of this
%                    function).
%       - xstddev: standard deviation (but only if wndwtype is @rectwin).
%
% 
%
...
% variable, MAKE SURE (!) this one is evenly spaced
...
%% If output is specificed, also gives the standard deviation

xstddev = std(xmatrix, 1);
